...
if(isset($_POST['delete'])){
   ...
   $delete_wishlist_item = $conn->prepare("DELETE FROM `clientswishlist` WHERE id = ?");
   $delete_wishlist_item->execute([$wishlist_id]);
}

if(isset($_GET['delete_all'])){
   $delete_wishlist_item = $conn->prepare("DELETE FROM `clientswishlist` WHERE user_id = ?");
...
<section class="products">
   ...
   <?php
      $grand_total = 0;
      $select_wishlist = $conn->prepare("SELECT * FROM `clientswishlist` WHERE user_id = ?");
   ...
   <form action="" method="post" class="box">
      ...
      <img src="uploaded_img/<?= $fetch_wishlist['Product_Image']; ?>" alt="">
      <div class="name"><?= $fetch_wishlist['Product_Name']; ?></div>
      <div class="flex">
         <div class="price">R<?= $fetch_wishlist['Product_Price']; ?></div>
   ...
   <div class="wishlist-total">
      ...
      <a href="wishlist.php?delete_all" class="delete-btn <?= ($grand_total > 1)?'':'disabled'; ?>">delete all items</a>
   </div>
